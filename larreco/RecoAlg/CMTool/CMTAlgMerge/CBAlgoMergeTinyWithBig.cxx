...
namespace cmtool {
  ...
  //----------------------------------------------------------------
  {
    ...
    if (_debug)
      std::cout << "MergeTinyWithBig. One cluster has " << cluster1.GetNHits()
                << " hits, the other has " << cluster2.GetNHits() << " hits." << std::endl;
    ...
    if (cluster1.GetNHits() > _min_hits_small && cluster1.GetNHits() < _max_hits_small)
      is_1_small = true;
    //if the second cluster counts as "small"
    if (cluster2.GetNHits() > _min_hits_small && cluster2.GetNHits() < _max_hits_small)
      is_2_small = true;
    if (cluster1.GetNHits() > _min_hits_big && cluster1.GetNHits() < _max_hits_big) is_1_big = true;
    if (cluster2.GetNHits() > _min_hits_big && cluster2.GetNHits() < _max_hits_big) is_2_big = true;

    if (_debug)
      std::cout << "is_1_small, is_1_big, is_2_small, is_2_big are: " << is_1_small << ", "
                << is_1_big << ", " << is_2_small << ", " << is_2_big << std::endl;

    //if neither of the clusters is small don't merge
    if (!is_1_small && !is_2_small) return false;
    //if neither of the clusters is big, don't merge
    if (!is_1_big && !is_2_big) return false;
    //if both are small, don't merge
    if (is_1_small && is_2_small) return false;
    //if both are big, don't merge
    if (is_1_big && is_2_big) return false;

    if (_debug) std::cout << "Looks like one of them is big, and one is small." << std::endl;
    ...
    for (unsigned int i = 0; i < npoints1; ++i) {
      ...
      for (unsigned int j = 0; j < npoints2; ++j) {
        ...
        if (_debug) { std::cout << "two polygon points dist2 is " << distsqrd << std::endl; }
        if (distsqrd < _dist_sqrd_cut) return true;
      }
    }
