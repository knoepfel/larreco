...
namespace cmtool {
  ...
  //---------------------------------------------------------------------------
  bool
  CBAlgoCenterOfMass::Bool(const ::cluster::ClusterParamsAlg& cluster1,
  ...
  //---------------------------------------------------------------------------
  {
    ...
    if ((cluster1.GetHitVector().size() > _minHits) and (cluster2.GetHitVector().size() < _maxHits))
      small = 2;
    else if ((cluster2.GetHitVector().size() > _minHits) and
             (cluster1.GetHitVector().size() < _maxHits))
      small = 1;
    else
      return false;
    ...
    double COM_w_rot = (COM_w_s - start_w) * cos(direc_angle * 3.14 / 180.) +
                       (COM_t_s - start_t) * sin(direc_angle * 3.14 / 180.);
    double COM_t_rot = -(COM_w_s - start_w) * sin(direc_angle * 3.14 / 180.) +
                       (COM_t_s - start_t) * cos(direc_angle * 3.14 / 180.);
    ...
    if (_COMinConeAlg and (COM_w_rot < length * _lengthReach) and (COM_w_rot > 0) and
  ...
  //-----------------------
  void
  CBAlgoCenterOfMass::Report()
  //-----------------------
  {}

  double
  CBAlgoCenterOfMass::ShortestDistanceSquared(double point_x,
                                              double point_y,
                                              double start_x,
                                              double start_y,
                                              double end_x,
