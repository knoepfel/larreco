...
namespace cmtool {
  ...
  //----------------------------------------------------------------
  {
    ...
    if (cluster1.GetNHits() < _minHits || cluster2.GetNHits() < _minHits) return false;

    //if either cluster is too short, do not prohibit them
    if (cluster1.GetParams().length < _min_length || cluster2.GetParams().length < _min_length)
      return false;
    ...
    if (_debug) {
      if (!compatible)
        ...
        std::cout << "CBAlgoAngleIncompat is PROHIBITING clusters with angles " << angle1 << " and "
                  << angle2 << std::endl;
    }
