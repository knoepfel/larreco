...
namespace cmtool {
  ...
  //----------------------------------------------------------------
  {
    ...
    if (cluster1.GetHitVector().size() < _min_hits || cluster2.GetHitVector().size() < _min_hits)
      return false;

    bool FirstTrackSecondBlob = false;
    bool SecondTrackFirstBlob = false;

    if (IsStartTrack(cluster1) && IsOverlappingBlob(cluster2)) FirstTrackSecondBlob = true;
    if (IsStartTrack(cluster2) && IsOverlappingBlob(cluster1)) SecondTrackFirstBlob = true;
    ...
    if (_debug) {
      if (FirstTrackSecondBlob) {
        ...
        std::cout << "eigenvalue_principal = " << cluster1.GetParams().eigenvalue_principal
                  << std::endl;
        ...
        std::cout << "eigenvalue_principal = " << cluster2.GetParams().eigenvalue_principal
                  << std::endl;
      }
    }
