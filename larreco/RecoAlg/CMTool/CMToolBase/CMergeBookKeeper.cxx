...
namespace cmtool {
  ...
  CMergeBookKeeper::Merge(unsigned short index1, unsigned short index2)
  {
    ...
    if (_prohibit_merge.at(out_index1).at(out_index2 - out_index1))
      ...
      throw CMTException(
        Form("Clusters (%d,%d) correspond to output (%d,%d) which is prohibited to merge",
             index1,
             index2,
             out_index1,
             out_index2));
    ...
    for (size_t index = 0; index < out_index1; ++index) {
      ...
      _prohibit_merge.at(index).at(tmp_out_index1) =
        (_prohibit_merge.at(index).at(tmp_out_index1) ||
	...
	*/
	...
	std::cout<<Form("Inspecting1 : (%d,%zu) to (%zu,%zu)",
		 ...
		 << std::endl;
	*/
        _prohibit_merge.at(out_index1).at(in_index) =
          (_prohibit_merge.at(out_index1).at(in_index) ||
	...
	*/
        _prohibit_merge.at(out_index1).at(in_index) =
          (_prohibit_merge.at(out_index1).at(in_index + 1) ||
    ...
    for (size_t index = out_index1 + 1; index < out_index2; ++index) {
    ...
    for (size_t index = out_index2;
      ...
      for (size_t in_index = 0; in_index < _prohibit_merge.at(index).size(); ++in_index)
  ...
  CMergeBookKeeper::Report() const
  {
    ...
    for (auto const& bs : _prohibit_merge) {
      ...
      for (auto const& b : bs) {
        ...
        if (b)
          std::cout << "\033[93mT\033[00m ";
        else
          std::cout << "\033[95mF\033[00m ";
      }
      std::cout << std::endl;
    }
    std::cout << std::endl;
  ...
  CMergeBookKeeper::Combine(const CMergeBookKeeper& another)
  {
    ...
    if (my_result.size() != another.size()) {
      throw CMTException(
        Form("Input has an incompatible size (%zu != %zu)", my_result.size(), another.size()));
      return;
    }
    ...
    for (auto const& ares : another_result) {
      ...
      for (auto const& res_index : ares) {
        ...
        for (auto const& orig_index : my_result.at(res_index)) {
          ...
          if (target == orig_index)
            continue;

          else
            this->Merge(target, orig_index);
        }
      }
    }
  }
