...
getColor(int showerID)
{
  ...
  if (showerColor == 10)
    showerColor = 28;
  else if (showerColor > 10)
    showerColor += 27; // start again with color 38
  return showerColor;
}
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    for (int i = 0; i < nplanes; ++i) {
      for (int j = 0; j < nstages; ++j) {
        ...
        TPCview[i][j] =
          new TH2F(name.str().c_str(), title.str().c_str(), 500, 0, 5000, 500, 0, 10000);
    ...
    for (int i = 0; i < nstages; ++i) {
      ...
      for (int j = 0; j < nplanes; ++j) {
        ...
        for (int k = 0; k < (int)Envelope->size(); ++k) {
          ...
          if (k % 2 == 0)
            xx[(int)floor(vtxcnt / 2)] = Envelope->at(k);
          else
            yy[(int)floor(vtxcnt / 2)] = Envelope->at(k);
