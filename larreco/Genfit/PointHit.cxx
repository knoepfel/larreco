...
genf::PointHit::PointHit() : SpacepointRecoHit(NparHitRep) {}

genf::PointHit::PointHit(TVector3 point, double res) : SpacepointRecoHit(NparHitRep)
{
...
genf::PointHit::PointHit(TVector3 point, std::vector<double>& res) : SpacepointRecoHit(NparHitRep)
{
                           ...
                           const Double_t& dist)
{
  if (dynamic_cast<const genf::RKTrackRep*>(stateVector) != nullptr) {
    ...
    Double_t C =
      0.0136 / betac * sqrt(dist / 14.0) * (1 + 0.038 * log(dist / 14.0)); // EC, 2-Jan-2012.
